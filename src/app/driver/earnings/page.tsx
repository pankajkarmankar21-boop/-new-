...
import type { DriverEarningsBookingItem } from "@/types/joined";

export default function DriverEarningsPage() {
  const supabase = createClient();
  const [loading, setLoading] = useState(true);
  const [jobs, setJobs] = useState<DriverEarningsBookingItem[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        ...
        .returns<DriverEarningsBookingItem[]>();

      const list = data || [];
      setJobs(list);
      setTotalEarnings(list.reduce((s, j) => s + Number(j.final_amount), 0));

      const now = new Date();
      const thisMonth = list.filter((j) => {
        const d = new Date(j.updated_at);
        return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
      });
      setMonthEarnings(thisMonth.reduce((s, j) => s + Number(j.final_amount), 0));
      setLoading(false);
    }
    load();
  }, [supabase]);
