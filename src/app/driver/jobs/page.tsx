...
import type { DriverJobListItem } from "@/types/joined";
...
export default function DriverJobsPage() {
  ...
  const [active, setActive] = useState<DriverJobListItem[]>([]);
  const [history, setHistory] = useState<DriverJobListItem[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: bookings } = await supabase
        ...
        .returns<DriverJobListItem[]>();

      const list = bookings || [];
      setActive(list.filter((b) => b.status !== "completed" && b.status !== "cancelled" && b.status !== "rejected"));
      setHistory(list.filter((b) => b.status === "completed"));
      setLoading(false);
    }
    load();
  }, [supabase]);
            ...
            {active.map((job) => (
              <Link key={job.id} href={`/driver/jobs/${job.id}`} className="glass-card flex items-center justify-between p-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{job.farmers?.village} · {formatDateMarathi(job.booking_date)}</p>
