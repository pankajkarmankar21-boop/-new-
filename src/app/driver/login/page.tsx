...
export default function DriverLoginPage() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-gradient-to-br from-earth-600 via-earth-500 to-amber-600 px-6 py-10">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass-card mt-6 flex flex-1 flex-col p-6">
        <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-earth-500 to-amber-600 shadow-lg">
          {step === "mobile" ? <Phone className="h-8 w-8 text-white" /> : <ShieldCheck className="h-8 w-8 text-white" />}
        </div>

        {step === "mobile" ? (
          <>
            <h1 className="text-2xl font-bold text-gray-900">ड्रायव्हर लॉगिन</h1>
            ...
            <button onClick={handleSendOtp} disabled={loading} className="btn-primary mt-auto !bg-gradient-to-br !from-earth-600 !to-amber-600">
          ...
          <>
            ...
            <button onClick={handleVerifyOtp} disabled={loading} className="btn-primary !bg-gradient-to-br !from-earth-600 !to-amber-600">
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "पडताळणी करा"}
            </button>

            <button onClick={handleSendOtp} disabled={resendTimer > 0} className="mt-4 text-sm font-medium text-earth-700 disabled:text-gray-400">
