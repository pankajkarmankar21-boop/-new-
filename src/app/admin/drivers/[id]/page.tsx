...
export default function AdminFarmerDetailPage() {
  ...
  async function load() {
    const farmerRes = await supabase.from("farmers").select("*").eq("id", farmerId).single();
  ...
  async function handleApprove() {
    setProcessing(true);
    const { error } = await (supabase.from("farmers") as any).update({ approval_status: "approved", rejection_reason: null }).eq("id", farmerId);
    setProcessing(false);
  ...
  async function handleReject() {
    ...
    setProcessing(true);
    const { error } = await (supabase.from("farmers") as any).update({ approval_status: "rejected", rejection_reason: rejectReason }).eq("id", farmerId);
    setProcessing(false);
    ...
    <main className="min-h-screen bg-gray-50 px-5 py-6">
      <div className="mx-auto max-w-md">
        ...
        <div className="glass-card mb-4 p-5">
          ...
          <p className="mb-1 text-sm text-gray-700"><span className="text-gray-400">पत्ता:</span> {farmer.address}, {farmer.village}</p>
        ...
        <div className="glass-card mb-4 p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <img src={farmer.aadhar_front_url} alt="Aadhar Front" className="rounded-xl border border-gray-200" />
            <img src={farmer.aadhar_back_url} alt="Aadhar Back" className="rounded-xl border border-gray-200" />
          </div>
          {farmer.aadhar_number && <p className="mt-2 text-sm text-gray-700"><span className="text-gray-400">आधार:</span> {farmer.aadhar_number}</p>}
        </div>

        <div className="glass-card mb-4 p-5">
          <h2 className="mb-3 text-sm font-bold text-gray-700">शेतीची माहिती</h2>
          {farms.length === 0 ? (
            <p className="text-sm text-gray-400">कोणतीही शेती नोंद नाही</p>
          ) : (
            <div className="flex flex-col gap-2">
              {farms.map((f: any) => (
                <div key={f.id} className="rounded-xl bg-gray-50 px-3 py-2 text-sm">
                  <p><span className="text-gray-400">गाव:</span> {f.village}</p>
                  <p><span className="text-gray-400">क्षेत्र:</span> {f.area_acre} एकर</p>
                </div>
              ))}
            </div>
          )}
