...
export default function AdminLoginPage() {
  ...
  async function handleVerifyOtp() {
    ...
    if (profile?.role !== "admin") {
      ...
      setLoading(false);
      toast.error("हा नंबर Admin म्हणून नोंदणीकृत नाही");
      return;
    }
  ...
  return (
    <main className="flex min-h-screen flex-col bg-gradient-to-br from-sky-600 via-blue-600 to-indigo-700 px-6 py-10">
      <Link href="/" className="mb-6 flex h-10 w-10 items-center justify-center rounded-full bg-white/20 backdrop-blur">
        <ArrowLeft className="h-5 w-5 text-white" />
      </Link>

      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="glass-card mt-6 flex flex-1 flex-col p-6">
        <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-500 to-blue-600 shadow-lg">
          <ShieldCheck className="h-8 w-8 text-white" />
        </div>

        {step === "mobile" ? (
          <>
            <h1 className="text-2xl font-bold text-gray-900">ॲडमिन लॉगिन</h1>
            <p className="mt-1 mb-6 text-sm text-gray-500">नोंदणीकृत Admin मोबाईल नंबर टाका</p>
            ...
            <div className="mb-6 flex items-center gap-2">
              ...
              <input
                ...
                placeholder="9876543210" className="input-field flex-1" autoFocus
              />
            </div>

            <button onClick={handleSendOtp} disabled={loading} className="btn-primary mt-auto !bg-gradient-to-br !from-sky-600 !to-blue-700">
          ...
          <>
            ...
            <button onClick={handleVerifyOtp} disabled={loading} className="btn-primary !bg-gradient-to-br !from-sky-600 !to-blue-700">
              {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "पडताळणी करा"}
            </button>

            <button onClick={handleSendOtp} disabled={resendTimer > 0} className="mt-4 text-sm font-medium text-sky-700 disabled:text-gray-400">
