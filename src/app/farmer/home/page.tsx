...
export default function FarmerHomePage() {
  ...
  const [currentBooking, setCurrentBooking] = useState<ActiveBookingWithDriver | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: activeBooking } = await supabase
        ...
        .limit(1)
        .maybeSingle()
        .returns<ActiveBookingWithDriver>();
            ...
            {currentBooking.drivers ? (
              <div className="flex items-center justify-between rounded-2xl bg-primary-50 px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{currentBooking.drivers.full_name}</p>
                    <p className="text-xs text-gray-500">{currentBooking.drivers.mobile_number}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <a href={`tel:${currentBooking.drivers.mobile_number}`} className="flex h-9 w-9 items-center justify-center rounded-full bg-primary-600">
