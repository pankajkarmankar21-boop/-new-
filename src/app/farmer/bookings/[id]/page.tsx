...
import type { FarmerBookingDetail, FarmerBookingDetailItem } from "@/types/joined";
...
export default function FarmerBookingDetailPage() {
  ...
  const [booking, setBooking] = useState<FarmerBookingDetail | null>(null);
  const [items, setItems] = useState<FarmerBookingDetailItem[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: itemsData } = await supabase
        ...
        .returns<FarmerBookingDetailItem[]>();
      setItems(itemsData || []);
      setLoading(false);
    }
    load();
