...
import type { FarmerBookingListItem } from "@/types/joined";
...
export default function FarmerBookingsPage() {
  ...
  const [bookings, setBookings] = useState<FarmerBookingListItem[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data, count } = await query.returns<FarmerBookingListItem[]>();
      setBookings(data || []);
