...
import type { PaymentWithInvoiceDetails } from "@/types/joined";

type InvoiceFarmer = Pick<Farmer, "full_name" | "mobile_number" | "village">;

export default function FarmerInvoicesPage() {
  const supabase = createClient();
  const [loading, setLoading] = useState(true);
  const [payments, setPayments] = useState<PaymentWithInvoiceDetails[]>([]);
  const [farmer, setFarmer] = useState<InvoiceFarmer | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: farmerData } = await supabase
        .from("farmers")
        .select("full_name, mobile_number, village")
        .eq("id", user.id)
        .single();
      ...
      const { data } = await supabase
        ...
        .returns<PaymentWithInvoiceDetails[]>();
  ...
  function downloadInvoice(payment: PaymentWithInvoiceDetails) {
    ...
    doc.save(`${payment.invoice_number ?? payment.id}.pdf`);
  }
